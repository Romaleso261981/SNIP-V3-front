...
export default async function BottomDescription({
  ...
  const { bottomFirstText, bottomSecondText, separatorImage, gallery } = data;
  ...
  return (
    <div className='flex flex-col justify-center items-center'>
      ...
      <div className='flex md:hidden'>
        <Image
          src={getStrapiMedia(gallery[1].images[0].url) || ""}
          width={2400}
          height={gallery[1].images[0].height}
          objectFit='cover'
          alt='Separator'
          loading='lazy'
        />
      ...
      <div className='w-full flex flex-col justify-center gap-3 text-black items-center px-2 py-10 sm:w-3/4 md:w-2/6 md:justify-between md:px-3'>
        <p className='py-10 '>{bottomFirstText}</p>
        <div className='w-2/4 border-b border-b-gold' />
        <p className='py-10'>{bottomSecondText}</p>
      </div>
    </div>
  );
}
